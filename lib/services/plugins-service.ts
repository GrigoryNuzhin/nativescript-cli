...
export class PluginsService implements IPluginsService {
	...
	public async add(plugin: string): Promise<void> {
		...
		if (realNpmPackageJson.nativescript) {
			...
			try {
				...
				// Revert package.json
				this.$projectDataService.removeProperty(this.$projectData.projectDir, this.$pluginVariablesService.getPluginVariablePropertyName(pluginData.name));
	...
	private getInstalledFrameworkVersion(platform: string): string {
		let platformData = this.$platformsData.getPlatformData(platform);
		const frameworkData = this.$projectDataService.getValue(this.$projectData.projectDir, platformData.frameworkPackageName);
