...
export class PlatformService implements IPlatformService {
	...
	private async addPlatformCore(platformData: IPlatformData, frameworkDir: string): Promise<string> {
		...
		this.$projectDataService.setValue(this.$projectData.projectDir, platformData.frameworkPackageName, frameworkPackageNameData);
	...
	private async getPathToPlatformTemplate(selectedTemplate: string, frameworkPackageName: string): Promise<any> {
		if (!selectedTemplate) {
			...
			const nativescriptPlatformData = this.$projectDataService.getValue(this.$projectData.projectDir, frameworkPackageName);
	...
	public removePlatforms(platforms: string[]): void {
		...
		_.each(platforms, platform => {
			...
			this.$projectDataService.removeProperty(this.$projectData.projectDir, platformData.frameworkPackageName);
	...
	private async updatePlatform(platform: string, version: string): Promise<void> {
		...
		let data = this.$projectDataService.getValue(this.$projectData.projectDir, platformData.frameworkPackageName);
