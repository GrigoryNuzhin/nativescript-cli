...
export class PluginVariablesService implements IPluginVariablesService {
	...
	public async savePluginVariablesInProjectFile(pluginData: IPluginData): Promise<void> {
		...
		if (!_.isEmpty(values)) {
			this.$projectDataService.setValue(this.$projectData.projectDir, this.getPluginVariablePropertyName(pluginData.name), values);
		}
	}

	public removePluginVariablesFromProjectFile(pluginName: string): void {
		this.$projectDataService.removeProperty(this.$projectData.projectDir, this.getPluginVariablePropertyName(pluginName));
	}
	...
	private createPluginVariableData(pluginData: IPluginData, pluginVariableName: string): IPluginVariableData {
		...
		const pluginVariableValues = this.$projectDataService.getValue(this.$projectData.projectDir, this.getPluginVariablePropertyName(pluginData.name));
